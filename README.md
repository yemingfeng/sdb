...
- 当有写入请求的时候，可以请求阶段中的任意节点。如果请求的节点是主节点，则直接发起提案。若请求的节点是从节点，则该节点会进行请求转发，将写入请求转发到主节点中。
...
**当需要使用读写分离的时候，可以创建多个 sdb grpc client，指定节点的 ip 和端口即可实现读写分离。**
