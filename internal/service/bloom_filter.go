...
var bloomFilterCollection = store.NewCollection(pb.DataType_BLOOM_FILTER)
...
func BFCreate(key []byte, n uint32, p float64) error {
	...
	if err := bloomFilterCollection.UpsertRow(&store.Row{
		Key:   key,
		Id:    key,
	...
	if err := PAdd(pb.DataType_BLOOM_FILTER, key, batch); err != nil {
		return err
	}
...
func BFDel(key []byte) error {
	...
	if err := PDel(pb.DataType_BLOOM_FILTER, key, batch); err != nil {
		return err
	}
